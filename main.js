...
ipcMain.on('input:keyboard', (event, data) => {
    ...
    if (win && win.scrutinizerView && !win.scrutinizerView.isDestroyed()) {
        ...
        // This mapping ensures common keys are correctly interpreted by Electron's sendInputEvent
        const keyMap = {
            'ArrowUp': 'Up',
            'ArrowDown': 'Down',
            'ArrowLeft': 'Left',
        ...
        // Forward all relevant key properties
        win.scrutinizerView.webContents.sendInputEvent({
            type: data.type, // 'keyDown' or 'keyUp'
            keyCode: keyCode, // The (potentially remapped) key identifier
            modifiers: data.modifiers || [], // e.g., ['shift', 'control']
            isAutoRepeat: data.isAutoRepeat || false,
            text: data.text || '', // The character generated by the key press
            unmodifiedText: data.unmodifiedText || '', // The character without modifiers
            code: data.code || '', // Physical key identifier (e.g., 'KeyA', 'Digit1')
            key: data.key || '' // Logical key identifier (e.g., 'a', '1', 'ArrowUp')
        });
    }
});
