...
- [ ] **B4 — Make the published OCR profile real (Fovea 84/Para 60/Near 62/Far 52).** _Code landed 2026-06-05 (parallel OCR-investment workflow); manual re-capture remaining._ The dead gate had **three** causes, not just DPR: (1) a tesseract.js-v7 API break — `ocrByRing` read a flat `data.words` that v7 removed, so it read **0 chars on every image**, even a perfect baseline; (2) DPR never pinned; (3) a 0-read recorded as a `0%` datum, only `console.warn` on mismatch.
    ...
    - **Remaining (Electron, manual):** `TEST_DPR=2 node scripts/validate-peripheral-ocr.js --freeze-baseline`, then `--capture` (mode 0) and `--mode 12 --capture`; commit the regenerated `ocr-baseline.json` + `ocr-accuracy-curve*.json`; update CHANGELOG with the real numbers; then **calibrate thresholds** (spec 85%/30pp vs. the documented ~76% disabled-vs-HUD foveal ceiling) against real data. Negative test: feed a DPR-1 capture → must exit 2, not score 0%.
